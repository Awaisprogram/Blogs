...
// Define the proper PageProps interface for Next.js
interface PageProps {
  params: Promise<{ id: string }>;
  searchParams?: Promise<{ [key: string]: string | string[] | undefined }>;
}

const getProjectData = (id: string): Blogs | undefined => {
  const blog = blogs.find((p) => p.id === parseInt(id));
  return blog as Blogs | undefined;
}

// Make the component async and use proper PageProps
async function BlogsData({ params }: PageProps) {
  // Await the params since they're now a Promise
  const { id } = await params;
  
  const blog = getProjectData(id);
